export function mapAmlObligations(db, input) {
  ...
  `).get();
    if (fatfSource)
        applicableSources.push(fatfSource);
  ...
  `).get();
    if (uncacSource)
        applicableSources.push(uncacSource);
    ...
    `).get();
        if (oecdSource)
            applicableSources.push(oecdSource);
    }
    // Get entity-type-specific provisions if requested
    let entityObligations = [];

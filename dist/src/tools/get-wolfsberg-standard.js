...
export function getWolfsbergStandard(db, input) {
    if (input.title) {
        const standard = db.prepare(`
      SELECT * FROM wolfsberg_standards
      WHERE title LIKE ?
    `).all(`%${input.title}%`).map((s) => ({
            ...s,
    ...
    // Return all
    const all = db.prepare('SELECT * FROM wolfsberg_standards ORDER BY publication_date DESC').all().map((s) => ({
        ...s,
        _citation: buildCitation(s.title, s.title, 'get_wolfsberg_standard', { title: s.title }),
    }));
    return {

...
export function seedWolfsberg(db: Database.Database): void {
  console.log('  Seeding Wolfsberg Group Standards...');

  const insertStd = db.prepare(
    `INSERT INTO wolfsberg_standards (title, version, publication_date, category, content, applicability, key_requirements)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  );

  const standards: Array<[string, string, string, string, string, string, string]> = [
    [
